...
func TestValidateInfo(t *testing.T) {
	t.Run("Summary30", func(t *testing.T) {
		...
		assertHasError(t, errs, "info.summary requires OpenAPI 3.1.x or 3.2.0")
	})

	t.Run("InvalidTOS", func(t *testing.T) {
		tos := "://bad"
		errs := validate.ValidateInfo(openapi.Info{TermsOfService: &tos}, openapi.Version312)
		assertHasError(t, errs, "info.termsOfService must be a URI")
	})
	...
	t.Run("ContactInvalidURL", func(t *testing.T) {
		...
		assertHasError(t, errs, "info.contact.url must be a URI")
	})
	...
	t.Run("ContactInvalidEmail", func(t *testing.T) {
		...
		assertHasError(t, errs, "info.contact.email must be an email address")
	})
	...
	t.Run("LicenseMissingName", func(t *testing.T) {
		...
		assertHasError(t, errs, "info.license.name is required")
	})
	...
	t.Run("LicenseInvalidURL", func(t *testing.T) {
		...
		assertHasError(t, errs, "info.license.url must be a URI")
	})
	...
	t.Run("LicenseIdentifier30", func(t *testing.T) {
		...
		assertHasError(t, errs, "info.license.identifier requires OpenAPI 3.1.x or 3.2.0")
	})
}
